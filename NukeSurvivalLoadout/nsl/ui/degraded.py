...
* The panic button stays available - it only flips ``PANIC_MODE`` in the
  ...
  completing.
* No ``import nuke``. Qt access only via :mod:`nsl.compat`.
...
class DegradedBanner(compat.QtWidgets.QWidget):
    ...
    Visually mirrors :class:`nsl.ui.banner.Banner` (the change-detected
...
def wire_degraded(panel: Any) -> None:
    ...
    * Enters degraded mode when EITHER the boot sequence recorded a
      critical failure (:func:`nsl.boot.self_recovery.boot_failed`)
...
def _activate_summary_tab(panel: Any) -> None:
    ...
    if tabs is None:
        return
    # nsl.ui.side_panel.TAB_SUMMARY is the canonical constant; import
