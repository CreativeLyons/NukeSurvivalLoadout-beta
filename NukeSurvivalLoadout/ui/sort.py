...
Qt imports go only via :mod:`NukeSurvivalLoadout.compat`. The
:class:`SortMode` enum and :data:`SORT_MODE_ORDER` are imported from
:mod:`NukeSurvivalLoadout.ui.grid_toolbar` (the toolbar widget is the
...
from NukeSurvivalLoadout.ui.grid_toolbar import SORT_MODE_ORDER, SortMode
...
# (``NukeSurvivalLoadout.ui.wiring.sort_state.wire_sort_state_lookup``) installs the
...
def _problem_state_for_key(
    ...
    *,
    ...
    :func:`NukeSurvivalLoadout.ui.state._derive_status_icon` runs from
    :meth:`NukeSurvivalLoadout.ui.panel.LoadoutPanel._set_pills_from_registry` so the
...
def build_sort_state_lookup(panel) -> StateLookup:
    ...
    Used by :func:`NukeSurvivalLoadout.ui.wiring.sort_state.wire_sort_state_lookup` to
...
def build_key_to_folder(panel) -> Callable[[str], Optional[str]]:
    ...
    pills (see :func:`NukeSurvivalLoadout.ui.filter_pipeline.wire_filter_pipeline`'s
