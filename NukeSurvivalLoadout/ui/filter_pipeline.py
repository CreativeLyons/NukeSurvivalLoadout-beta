...
# We share the search rule with :mod:`NukeSurvivalLoadout.ui.search_tags`
# rather than re-encoding it, so the case-insensitive substring rule
# stays in exactly one place.
from NukeSurvivalLoadout.ui.search_tags import match_query
...
def search_layer(query: Optional[str], plugin_name: str) -> bool:
    ...
    Delegates to :func:`NukeSurvivalLoadout.ui.search_tags.match_query` so the rule lives
...
def tag_layer_v1(
    ...
    :class:`NukeSurvivalLoadout.ui.search_tags.SearchTagsStrip` ships the system
    ...
    * ``apply_visibility``: optional callable ``list[str] -> None`` that
    ...
    def _on_state_recompute(self, visible: List[str]) -> None:
        ...
        if self._sort_mode is not None:
            ...
            # ``NukeSurvivalLoadout.ui.sort`` and re-imports the toolbar enum.
            from NukeSurvivalLoadout.ui.sort import (  # noqa: PLC0415
                SortableState,
