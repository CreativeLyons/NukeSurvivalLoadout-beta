...
The widget code imports Qt exclusively via :mod:`NukeSurvivalLoadout.compat` -
...
from NukeSurvivalLoadout import compat  # noqa: E402 - kept after pure helpers on purpose
from NukeSurvivalLoadout.ui._buttons import HybridTextButton  # noqa: E402
...
class _ClearGlyphButton(QtWidgets.QToolButton):
    ...
    :mod:`NukeSurvivalLoadout.ui.loadout_strip` works around. Pre-render both icons at
...
class SearchTagsStrip(QtWidgets.QWidget):
    ...
    * the Invert toggle state (``QToolButton`` with ``setCheckable(True)``).
    ...
    # call into ``NukeSurvivalLoadout.domain.panic.reset_global_to_default``.
