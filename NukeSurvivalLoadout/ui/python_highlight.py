...
Design constraints (shared with the rest of ``NukeSurvivalLoadout.ui``):

* Qt is reached **only** through :mod:`NukeSurvivalLoadout.compat` - never import
...
def _build_highlighter_class():
    ...
    Imports Qt via :mod:`NukeSurvivalLoadout.compat` on first call so the module
